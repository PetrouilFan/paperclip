...
The 60-second budget is per bounded step, not per chain. Each finalization step re-arms it, so a chain that is making progress through run-log finalize, terminal persistence, continuation summary, issue comment policy, interaction retry, issue release, review disposition, runtime state, and agent status is never declared abandoned for being slow. An independent 15-minute backstop still covers a chain that keeps renewing and never terminates. Per-step timings are recorded from the moment the provider settles, so the step that overran is identified from data rather than inferred.

Sweeping a run preserves the run's own recorded outcome. A run that already carries an `errorCode` keeps it; the deadline is recorded as a fact about the sweep, in the run's result metadata and in the recovery action evidence, not as a replacement cause. The run's `error` describes the fault, including the step that was in flight; the operator instruction belongs to `next_action` only. A recovery action reached this way records what could be established about the provider process — stopped, still running, or not establishable — instead of leaving ownership unverified, and a sweep that fails logs the error with its run identity rather than only noting that the run is still pending.
